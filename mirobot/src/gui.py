...
class DrawingRobotApp:
    ...
    def canvas_to_robot(self, canvas_x, canvas_y):
        return canvas_to_robot(canvas_x, canvas_y)

    def robot_to_canvas(self, robot_x, robot_y):
        return robot_to_canvas(robot_x, robot_y)
    ...
    def connect_robot(self):
        ...
        try:
            ...
            self.connected = True
            self.set_status(f"연결 완료: {SERIAL_PORT}, 속도 {ROBOT_SPEED}")
        except Exception as error:
            self.connected = False
            try:
    ...
    def move_robot(self, x, y, z, linear=True):
        ...
        x, y, z = self.robot_controller.move(x, y, z, linear=linear)
    ...
    def move_robot_measurement(self, x, y, z, log_file, start_time, linear=True):
        ...
        current_x, current_y, current_z = self.robot_controller.get_xyz()
    ...
    def check_robot(self):
        if not self.connected or not self.robot_controller.connected:
            messagebox.showwarning(
                "연결 필요",
                "먼저 로봇 연결 버튼을 누르세요.",
            )
